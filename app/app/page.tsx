...
import { trackSearchUsage } from '@/lib/usage-tracking';
import { addSearchHistoryEntry } from '@/lib/search-history';
import { useIsPaid } from '@/hooks/use-is-paid';
import { useIsMobile } from '@/hooks/use-mobile';
import { Popover, PopoverTrigger, PopoverContent } from '@/components/ui/popover';
import { AgGridWrapper, AgGridWrapperRef, HeaderClickInfo } from '@/components/ui/ag-grid-wrapper';
...
import { ConfigWizardModal } from '@/components/paywall/ConfigWizardModal';
...
import { getVerificationCountsForRow } from '@/lib/verifications';
import { buildMockRowsForTab } from '@/lib/mock-search';
import { seedMockVerificationsForRow } from '@/lib/mock-verifications';
...
const BadgeRenderer = (params: any) => {
  ...
  const isBlankNewRow = params.data?.__isNewRow && value === '';
  if (!value) return isBlankNewRow ? null : <span className="text-gray-400">N/A</span>;
  return (
...
const LinkRenderer = (params: any) => {
  ...
  const isBlankNewRow = params.data?.__isNewRow && (url === null || url === undefined || url === '');
...
const EmailRenderer = (params: any) => {
  const value = params.value;
  const isBlankNewRow = params.data?.__isNewRow && (value === null || value === undefined || value === '');
  if (!value) return isBlankNewRow ? null : <span className="text-gray-400">N/A</span>;
  return (
    <a
      href={`mailto:${value}`}
...
const SEARCH_SUGGESTION_CARDS = [
  {
    title: 'Find AI companies that are breaking through to new markets that specialize in edge AI',
    tags: ['Companies', 'AI', 'Startups'],
  },
  {
    title: 'Find companies founded by Stanford graduates that raised Series A in 2023',
    tags: ['Companies', 'Funding', 'Education'],
  },
  {
    title: 'Find market intel teams hiring for competitive research in the US',
    tags: ['People', 'Hiring'],
  },
];
...
const QUICK_SEARCH_SUGGESTIONS = [
  ...
  'What are recent React CVEs?',
  'Sales leaders at US fintech companies',
];
...
function SearchHeader({
  ...
  const filteredQuickSuggestions = normalizedQuery
    ? QUICK_SEARCH_SUGGESTIONS.filter((item) => item.toLowerCase().includes(normalizedQuery))
  ...
  const handleSuggestionScroll = (direction: 'left' | 'right') => {
    ...
    if (!container) return;
    const scrollAmount = Math.max(240, Math.round(container.clientWidth * 0.85));
    container.scrollBy({
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
      <div className="flex flex-col lg:flex-row gap-4">
        ...
        <div className="flex-1 flex gap-3">
          <div className="flex-1 relative">
            ...
            />
            {showSuggestions && filteredQuickSuggestions.length > 0 && (
              <div className="absolute left-0 right-0 top-full mt-2 rounded-xl border border-gray-200 bg-white shadow-lg overflow-hidden z-20">
          ...
          <Popover open={searchPopoverOpen} onOpenChange={setSearchPopoverOpen}>
            <PopoverTrigger asChild>
              <button
                ...
                disabled={isPaid ? (isSearching || isCooldown || !query.trim()) : false}
            ...
            {!isPaid && (
              <PopoverContent
                ...
                onMouseLeave={() => setSearchPopoverOpen(false)}
              ...
              >
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-gray-500">
                    Unlock to run searches and get full results.
                  </p>
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="w-full rounded-lg bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2"
                  >
                    Unlock Search
                  </button>
                </div>
      ...
      <div className="mt-5 space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            <p className="text-sm font-semibold text-gray-700">Suggested searches</p>
            <span className="text-xs text-gray-400">Click to populate</span>
          </div>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => handleSuggestionScroll('left')}
              className="h-8 w-8 rounded-full border border-gray-200 text-gray-600 hover:text-indigo-600 hover:border-indigo-200 hover:bg-indigo-50 transition-colors"
        ...
        <div
          ...
          className="flex gap-3 overflow-x-auto pb-2 pr-2 scroll-smooth scrollbar-hide"
        >
          {SEARCH_SUGGESTION_CARDS.map((card) => (
            <button
              key={card.title}
              type="button"
              onClick={() => handleSuggestionPick(card.title)}
              className="min-w-[260px] max-w-[320px] shrink-0 text-left rounded-xl border border-gray-200 bg-gray-50/40 hover:bg-white hover:border-indigo-200 transition-all p-4 shadow-sm"
            >
              <p className="text-sm font-semibold text-gray-900">
                {card.title}
              </p>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="flex items-center gap-2 min-w-0 flex-wrap">
          ...
          <button
            onClick={onVerify}
            disabled={!hasSelection}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-1"
            aria-label={`Verify ${selectedCount} selected rows`}
            aria-disabled={!hasSelection}
          >
        ...
        <div className="ml-auto flex items-center justify-end gap-3 min-w-0 flex-wrap lg:flex-nowrap">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <button
              type="button"
              onClick={() => setVerifyMessage('CRM connection coming soon.')}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-all bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 shrink-0"
            >
...
function ResultsPanel({ 
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      {verifyMessage && (
        <div className="px-4 py-2 bg-emerald-50 border-b border-emerald-100 text-emerald-800 text-sm">
          {verifyMessage}
        </div>
      )}
...
export default function Page() {
  ...
  const [verificationCounts, setVerificationCounts] = useState<Record<string, { count: number; bestStatus: any }>>({});
  ...
  }>({ open: false, field: '', headerName: '', rect: null, isCustomColumn: false, autoEditMode: false });
  ...
  useEffect(() => {
    ...
    isSubscribeOpenRef.current = isConfigWizardOpen || isSubscribeOpen;
  }, [isPaywallOpen, isConfigWizardOpen, isSubscribeOpen]);
  ...
  useEffect(() => {
    if (!isPaywallOpen && !isConfigWizardOpen && !isSubscribeOpen) return;
  ...
  const handleSuggestionSearch = useCallback(
    (value: string) => {
      ...
      setError(null);
      setQuotaError(false);
      setResolvedSearchMode('mock');
      setDemoBanner('Using demo results');

      const mockResults = buildMockResults(trimmedQuery);
    ...
    [buildMockResults, setResults, setSearchedTabs],
  );

  useEffect(() => {
    const queryParam = searchParams.get('query');
    if (!queryParam) return;
    if (historyQueryRef.current === queryParam) return;
    historyQueryRef.current = queryParam;

    const tabParam = searchParams.get('tab');
    if (tabParam && tabConfigs.some((tab) => tab.id === tabParam)) {
      activeTabRef.current = tabParam as TabId;
      setActiveTab(tabParam as TabId);
    }

    handleSuggestionSearch(queryParam);
  }, [searchParams, handleSuggestionSearch]);
  ...
  const handleOpenRow = useCallback((row: any) => {
    ...
    const rowId = row.id || row.name || row.title || '';
    if (rowId) {
      seedMockVerificationsForRow(activeTab, rowId, row);
      setVerificationCounts(getVerificationCountsForRow(activeTab, rowId));
    }
  }, [activeTab]);

  const handleShowVerifications = useCallback((fieldId: string) => {
    if (!sidebarState) return;
    const row = sidebarState.row;
    const rowId = row.id || row.name || row.title || String(Date.now());
    if (rowId) {
      const seeded = seedMockVerificationsForRow(sidebarState.tab, rowId, row);
      if (seeded) {
        setVerificationCounts(getVerificationCountsForRow(sidebarState.tab, rowId));
      }
    }
  }, [sidebarState]);
  ...
  // Instant Add Column - creates column immediately and opens rename popover
  const handleAddColumnInstant = useCallback(() => {
    ...
    let counter = 1;
    let fieldName = 'new-column';
    while (existingFields.has(fieldName)) {
      counter++;
      fieldName = `new-column-${counter}`;
    }
    
    const headerName = counter === 1 ? 'New Column' : `New Column ${counter}`;
    ...
    // Open the rename popover after a short delay to let the column render
    setTimeout(() => {
      // Find the header cell for the new column
      const headerCell = document.querySelector(`.ag-header-cell[col-id="${fieldName}"]`);
      if (headerCell) {
        ...
        if (api) {
          api.ensureColumnVisible(fieldName);
        }
      }
    }, 100);
  }, [activeTab, columnDefsByTab, setColumnDefsByTab, setResults, gridRef]);
  ...
  const handleAddRowInstant = useCallback(() => {
    const rowColumns = columnDefsByTab[activeTab] || [];
    const newRow: Record<string, any> = {
      id: createRowId(activeTab),
      __isNewRow: true,
    };

    rowColumns.forEach((col) => {
      if (!col.field || col.field === 'id') return;
      if (col.colId === 'select' || col.field === 'actions') return;
      newRow[col.field] = '';
    });
  ...
  }, [activeTab, columnDefsByTab, setResults]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-slate-100">
      ...
      <main className="max-w-[1600px] mx-auto px-6 py-6 space-y-4">
        ...
        <SearchHeader
          ...
          onSuggestionPick={handleSuggestionSearch}
        ...
        <ResultsPanel
          ...
          onAddColumn={handleAddColumnInstant}
          onAddRow={handleAddRowInstant}
          onMatch={toggleMatchPaintMode}
          onNotMatch={toggleNotMatchPaintMode}
          onExport={handleExport}
          columnDefsByTab={columnDefsByTab}
          onColumnHeaderClick={(info) => {
            const tabColumns = columnDefsByTab[activeTab] || [];
            const column = tabColumns.find((col) => col.field === info.field);
            const isCustom = (column as CustomColDef)?.__isCustom ?? false;
            setHeaderPopover({
              open: true,
              field: info.field,
        ...
        <RowDetailsSidebar
          ...
          tabId={sidebarState?.tab}
          rowId={sidebarState?.row?.id || sidebarState?.row?.name || sidebarState?.row?.title || ''}
          title={sidebarModel?.title || ''}
          link={sidebarModel?.link}
          fields={sidebarModel?.fields || []}
          summary={sidebarModel?.summary}
          onShowVerifications={handleShowVerifications}
        ...
        <ColumnHeaderPopover
          open={headerPopover.open}
          onClose={() => setHeaderPopover(prev => ({ ...prev, open: false, autoEditMode: false }))}
          field={headerPopover.field}
          headerName={headerPopover.headerName}
          rect={headerPopover.rect}
          isCustomColumn={headerPopover.isCustomColumn}
          autoEditMode={headerPopover.autoEditMode}
          onRename={(newName) => {
            if (headerPopover.isCustomColumn) {
              handleColumnNameChange(activeTab, headerPopover.field, newName);
            }
          }}
          onDelete={() => {
            if (headerPopover.isCustomColumn) {
              // Remove column from definitions
              setColumnDefsByTab(prev => ({
                ...prev,
                [activeTab]: prev[activeTab].filter(col => col.field !== headerPopover.field),
              }));
              // Remove field from row data
              setResults(prev => ({
                ...prev,
                [activeTab]: prev[activeTab].map(row => {
                  const { [headerPopover.field]: _, ...rest } = row;
                  return rest;
                }),
              }));
            }
          ...
          onSort={(direction) => {
            ...
            if (api) {
              api.applyColumnState({
                state: [{ colId: headerPopover.field, sort: direction }],
      ...
      <PaywallModal
        ...
        onOpenSubscribe={() => setIsConfigWizardOpen(true)}
        isSubscribeOpen={isConfigWizardOpen || isSubscribeOpen}
      />
      <ConfigWizardModal
